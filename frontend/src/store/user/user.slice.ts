...
import camelize from "../../utils/snakeToCamelCase";
...
>("user/registerUser", async (credentials, { rejectWithValue }) => {
  try {
    const response = await api.post<User>("accounts/register/", credentials);
...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentUser(_, action: PayloadAction<User>): CurrentUser {
